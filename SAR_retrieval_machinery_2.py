...
def wcm_jac_(A, V1, B, V2, alpha, C, theta=23):
    ...
    return (
        veg + soil,
        [der_dA, der_dB, der_dC, der_dalpha, der_dV1, der_dV2]
    )
...
def fwd_model_(x, svh, svv, theta):
    ...
    alpha = x[6 : (6 + n_obs)]
    lai = x[(6 + n_obs) :]
    sigma_vv, dvv = wcm_jac_(A_vv, lai, B_vv, lai, C_vv, alpha, theta=theta)
    sigma_vh, dvh = wcm_jac_(A_vh, lai, B_vh, lai, C_vh, alpha, theta=theta)
...
def cost_obs_(x, svh, svv, theta, unc=0.5):
    ...
    jac = np.concatenate(
        [##[der_dA, der_dB, der_dC, der_dR, der_dalpha, der_dV1, der_dV2]
            ...
            dvv[3] * diff_vv + dvh[3] * diff_vh,  # alpha
...
def cost_prior_(x, svh, svv, theta, prior_mean, prior_unc):
    ...
    cost1 = prior_cost[(6+n_obs):].sum() # LAI cost
